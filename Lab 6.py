...
plot_time_series(outside, 'Sensors Outside Data')
plot_time_series(inside, 'Sensors Inside Data')
...
time_outside1 = np.arange(30, len(outside1))  # Start from the 31st row
data_outside1 = outside1.iloc[30:, 2]  # 5th column (index 4), starting from the 31st row

time_outside2 = np.arange(30, len(outside2))  # Start from the 31st row
data_outside2 = outside2.iloc[30:, 7]  # 2nd column (index 1), starting from the 31st row

# Load the first inside dataset (inside1)
inside1 = pd.read_csv('insidedata.csv', skiprows=0)

# Load the second inside dataset (inside2)
inside2 = pd.read_csv('SensorsInside.csv', skiprows=1)

# Generate time series (assuming 1 entry per second) and remove the first 30 entries for both inside datasets
time_inside1 = np.arange(30, len(inside1))  # Start from the 31st row
data_inside1 = inside1.iloc[30:, 2]  # 5th column (index 4), starting from the 31st row

time_inside2 = np.arange(30, len(inside2))  # Start from the 31st row
data_inside2 = inside2.iloc[30:, 7]  # 2nd column (index 1), starting from the 31st row

# Load the inside dataset for final inside comparison and remove the first 30 entries
inside = pd.read_csv('SensorsInside.csv', skiprows=1)
time_inside = np.arange(30, len(inside))  # Start from the 31st row
data_inside = inside.iloc[30:, 1]  # 2nd column (index 1), starting from the 31st row

# Find the smallest length to truncate all data to the same size
min_length = min(len(data_outside1), len(data_outside2), len(data_inside1), len(data_inside2), len(data_inside))

# Truncate all datasets to the same size (min_length)
time_outside1 = time_outside1[:min_length]
time_outside2 = time_outside2[:min_length]
time_inside1 = time_inside1[:min_length]
time_inside2 = time_inside2[:min_length]
...
# Plotting all the data on one graph
plt.figure(figsize=(10, 6))

# Plot outside temperatures
plt.plot(time_outside1, data_outside1, label='Other Outside (outside1)', color='blue')
plt.plot(time_outside2, data_outside2, label='Self Outside (outside2)', color='red')

# Plot inside temperatures
plt.plot(time_inside1, data_inside1, label='Other Inside (inside1)', color='green')
plt.plot(time_inside2, data_inside2, label='Self Inside (inside2)', color='orange')
...
# Load the first outside dataset (outside1)
outside1 = pd.read_csv('outsidedata.csv', skiprows=0)

# Load the second outside dataset (outside2)
outside2 = pd.read_csv('SensorsOutside.csv', skiprows=1)

# Generate time series (assuming 1 entry per second) and remove the first 30 entries for both datasets
time_outside1 = np.arange(30, len(outside1))  # Start from the 31st row
data_outside1 = outside1.iloc[30:, 3]  # 5th column (index 4), starting from the 31st row

time_outside2 = np.arange(30, len(outside2))  # Start from the 31st row
data_outside2 = outside2.iloc[30:, 8]  # 2nd column (index 1), starting from the 31st row
...
# Generate time series (assuming 1 entry per second) and remove the first 30 entries for both inside datasets
time_inside1 = np.arange(30, len(inside1))  # Start from the 31st row
data_inside1 = inside1.iloc[30:, 3]  # 5th column (index 4), starting from the 31st row

time_inside2 = np.arange(30, len(inside2))  # Start from the 31st row
data_inside2 = inside2.iloc[30:, 8]  # 2nd column (index 1), starting from the 31st row
